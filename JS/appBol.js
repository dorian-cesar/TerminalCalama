...
formulario.addEventListener("submit", (e) => {
  ...
  traerDatos(idIn)
    ...
    .then(() => {
      alert(
        "El ticket ha sido escaneado exitosamente y el casillero ha sido liberado."
      );
      // Limpiar formulario
      formulario.reset();
    })
...
function printBol() {
  ...
  const valorTotal = parseFloat(
    document.querySelector("#valorTotal").textContent.replace("$", "").trim()
  );
    ...
    success: function (response) {
      try {
        ...
        if (response.respuesta === "OK") {
          ...
          if (!ventanaImpr) {
            alert(
              "Por favor, habilite las ventanas emergentes para visualizar el PDF."
            );
          } else {
...
function printComp() {
  ...
  const casillero = document.querySelector(
    "#tabla-body tr:nth-child(1) td:nth-child(2)"
  ).innerText;
  const fechaEntrada = document.querySelector(
    "#tabla-body tr:nth-child(2) td:nth-child(2)"
  ).innerText;
  const fechaSalida = document.querySelector(
    "#tabla-body tr:nth-child(3) td:nth-child(2)"
  ).innerText;
  const tiempoOcupado = document.querySelector(
    "#tabla-body tr:nth-child(4) td:nth-child(2)"
  ).innerText;
  const valorPorDia = document.querySelector(
    "#tabla-body tr:nth-child(5) td:nth-child(2)"
  ).innerText;
  const valorTotal = document.querySelector(
    "#tabla-body tr:nth-child(6) td:nth-child(2)"
  ).innerText;
  const talla = document.querySelector(
    "#tabla-body tr:nth-child(7) td:nth-child(2)"
  ).innerText;
